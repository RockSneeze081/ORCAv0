...
- [x] Test suite (97 tests, ~99% line coverage on the tested modules) + CI running it on every push
...
python -m coverage run --source=. --omit="tests/*" -m pytest tests/ -q && python -m coverage report -m
